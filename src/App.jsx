...
const App = () => {
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col bg-gradient-to-b min-h-screen flex flex-col">
      {/*  */}
  ...
  <Swiper
    ...
    pagination={{
      clickable: true,
      el: ".custom-pagination", // Custom selector for pagination
    }}
    spaceBetween={16}
    slidesPerView={1}
    breakpoints={{
  ...
  >
    {gradients.map(({ gradient, name, textGradient }, index) => (
      <SwiperSlide key={index}>
        <Card className="overflow-hidden rounded-lg transition-all duration-300 bg-slate-900/50 backdrop-blur-sm border border-slate-800 hover:border-slate-700 hover:bg-slate-900/80">
          <CardContent className="p-6">
            <div className={`h-32 rounded-lg mb-4 ${gradient}`} />
          ...
          <CardFooter className="p-4 flex justify-between border-t border-slate-800 bg-slate-900/30">
            ...
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setCurrentGradient(textGradient)}
              className="text-slate-300 hover:text-slate-100 hover:bg-slate-800/50"
              aria-label={`Preview gradient for ${name}`}
            >
  ...
  <div className="custom-pagination mt-6 flex justify-center"></div> {/* Custom pagination container */}
</main>


      {/* Footer */}
      <footer className="mb-12 dark:border-gray800">
        <div className="container mb-8 mx-auto px118 sm:px6 lg:px8 py6 text-center">
          Made by{" "}
          <a
            href="https://github.com/ShaiBatonya"
            target="_blank"
